...
MAX_WORKERS = int(os.getenv("VAHAN_MAX_WORKERS", "8"))
CACHE_DIR = os.getenv("VAHAN_CACHE_DIR", "vahan_cache")
CACHE_TTL_SECONDS = int(os.getenv("VAHAN_CACHE_TTL_SEC", str(3600)))  # 1 hour default
...
_GLOBAL_PROXY: Optional[Dict[str, str]] = None
_MANUAL_COOKIE: Optional[Dict[str, str]] = None
_PLAYWRIGHT_FALLBACK_ENABLED = PLAYWRIGHT_AVAILABLE
...
def _cache_key(url: str) -> str:
    return os.path.join(CACHE_DIR, f"{hashlib.sha256(url.encode()).hexdigest()}.pkl")
...
def save_to_cache(url: str, data: Any) -> None:
    if not data or (isinstance(data, (list, dict)) and not len(data)):
        return
    try:
...
def clear_cache():
    try:
        ...
        logger.info(Fore.GREEN + "🧹 Cache cleared successfully")
    except Exception as e:
        logger.error(Fore.RED + f"Cache clear failed: {e}")

# ---------------- CONFIG HELPERS ----------------
def set_proxy(proxy_dict: Dict[str, str]):
...
def set_manual_cookie(cookie_dict: Dict[str, str]):
    global _MANUAL_COOKIE
    _MANUAL_COOKIE = cookie_dict
    session.cookies.update(cookie_dict)
    logger.info(Fore.CYAN + "Manual cookies set.")


def enable_playwright_fallback(enable: bool):
    global _PLAYWRIGHT_FALLBACK_ENABLED
    if enable and not PLAYWRIGHT_AVAILABLE:
        logger.warning(Fore.YELLOW + "Playwright not installed; cannot enable fallback.")
    _PLAYWRIGHT_FALLBACK_ENABLED = bool(enable)
...
def build_params(from_year: int, to_year: int, state_code: str = "", rto_code: str = "0",
    ...
    params = {
        "fromYear": from_year, "toYear": to_year, "stateCode": state_code,
        "rtoCode": rto_code, "vehicleClasses": vehicle_classes,
        "vehicleMakers": vehicle_makers, "vehicleSubCategories": "",
        "vehicleEmissions": "", "vehicleFuels": "", "timePeriod": time_period,
        "vehicleCategoryGroup": "", "evType": "", "vehicleStatus": "",
        "vehicleOwnerType": "", "fitnessCheck": fitness_check, "vehicleType": vehicle_type
    }
...
def _random_headers() -> Dict[str, str]:
    return {
        "User-Agent": random.choice(RANDOM_UAS),
        "Accept": "application/json, text/plain, */*",
        "Referer": "https://analytics.parivahan.gov.in/analytics/vahanpublicreport?lang=en",
        "Origin": "https://analytics.parivahan.gov.in",
        "Connection": "keep-alive"
...
def _playwright_fetch(url: str, timeout: int = 60) -> Optional[Any]:
    ...
    try:
        ...
        with sync_playwright() as p:
            browser = p.chromium.launch(headless=True)
            ctx = browser.new_context(user_agent=random.choice(RANDOM_UAS))
            page = ctx.new_page()
            if _MANUAL_COOKIE:
                ctx.add_cookies([{"name": k, "value": v, "domain": "analytics.parivahan.gov.in"} for k, v in _MANUAL_COOKIE.items()])
            page.goto(url, wait_until="networkidle", timeout=timeout * 1000)
            body = page.evaluate("() => document.body.innerText")
            browser.close()
        try:
        ...
        logger.error(Fore.RED + f"Playwright error: {e}")
        return None
...
def _safe_get(url: str, timeout: int = int(DEFAULT_TIMEOUT)) -> Optional[Any]:
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            logger.info(Fore.GREEN + f"Fetching ({attempt}/{MAX_RETRIES}): {url}")
            resp = session.get(url, headers=_random_headers(), timeout=timeout, proxies=_GLOBAL_PROXY or {})
            ...
            logger.warning(Fore.YELLOW + f"Timeout {attempt}")
        except requests.RequestException as e:
            code = getattr(e.response, "status_code", None)
            logger.warning(Fore.YELLOW + f"HTTP {code}: {e}")
            if code and 400 <= code < 500 and code != 429:
                return None
        time.sleep(BACKOFF_FACTOR * (2 ** (attempt - 1)) + random.random())
    logger.error(Fore.RED + f"Max retries reached: {url}")
    return None

# ---------------- PUBLIC JSON WRAPPER ----------------
def get_json(path: str, params: Dict[str, Any], timeout: int = int(DEFAULT_TIMEOUT),
             use_cache: bool = True) -> Tuple[Optional[Any], str]:
    query = urlencode(params, doseq=True)
    url = f"{BASE.rstrip('/')}/{path.lstrip('/')}?{query}"
    ...
    data = _safe_get(url, timeout)
    if use_cache and data:
        save_to_cache(url, data)
    return data, url

# ---------------- BULK / RANGE HELPERS ----------------
def fetch_bulk(endpoints: List[str], common_params: Dict[str, Any],
               show_progress: bool = True, concurrent: bool = True) -> Dict[str, Any]:
    results = {}
    def _fetch(ep):
    ...
    if concurrent:
        with ThreadPoolExecutor(max_workers=min(MAX_WORKERS, len(endpoints))) as ex:
            for fut in tqdm(as_completed({ex.submit(_fetch, ep): ep for ep in endpoints}),
                            total=len(endpoints), desc="Bulk Fetch", disable=not show_progress):
                ep, data = fut.result()
                results[ep] = data
    else:
        for ep in tqdm(endpoints, desc="Bulk Fetch", disable=not show_progress):
            ep, data = _fetch(ep)
            results[ep] = data
    return results
...
def fetch_range(path: str, year_start: int, year_end: int, **kwargs) -> List[Any]:
    combined = []
    for y in range(year_start, year_end + 1):
        params = build_params(y, y, **{k: v for k, v in kwargs.items() if v is not None})
        data, _ = get_json(path, params)
        if isinstance(data, list):
            combined.extend(data)
        elif data:
...
# ---------------- UTILITIES ----------------
def preview_json(path: str, params: Dict[str, Any], n: int = 5):
    data, _ = get_json(path, params)
    if isinstance(data, list):
        return data[:n]
    if isinstance(data, dict):
...
def is_data_available(data: Any) -> bool:
    return bool(data and (not isinstance(data, (dict, list)) or len(data)))
