...
# -------------------------
all_dfs = {k: v for k, v in globals().items() if isinstance(v, pd.DataFrame)}

if not all_dfs:
    st.info("⚠️ No DataFrames available to export.")
else:
    ...
    with pd.ExcelWriter(excel_buffer, engine="xlsxwriter") as writer:
        # Cover sheet with key summaries
        cover_data = []
        ...
        cover_df.to_excel(writer, sheet_name="ALLL_MAXED_COVER", index=False)
        ws_cover = writer.sheets["ALLL_MAXED_COVER"]
        for i, col in enumerate(cover_df.columns):
            max_len = max(cover_df[col].astype(str).map(len).max(), len(col)) + 2
            ws_cover.set_column(i, i, max_len)
        ...
        for name, df in all_dfs.items():
            try:
                ...
                st.warning(f"Could not export {name}: {e}")

    excel_buffer.seek(0)

    # -------------------------
    # Capture charts in globals
    # -------------------------
    chart_images = []
    chart_names = [k for k in globals() if "fig" in k or "chart" in k]
    for cn in chart_names:
        ...
        try:
            ...
            buf.seek(0)
            chart_images.append((cn[:31], buf))
        except Exception as e:
            st.warning(f"Could not export chart {cn}: {e}")

    # -------------------------
    # Insert charts into Excel
    # -------------------------
    wb = load_workbook(excel_buffer)
    for sheet_name, img_buf in chart_images:
    ...
    # -------------------------
    st.download_button(
        label="📥 Download Ultimate ALLL-MAXED Export",
        data=final_buffer,
        file_name=f"ALLL_MAXED_EXPORT_{datetime.now().strftime('%Y%m%d_%H%M')}.xlsx",
        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
    )

    st.success("✅ ALLL-MAXED export ready! Cover sheet, all tables, charts, and summaries included.")
