# ==========================================================
# 🔁 AUTO RELOAD / REFRESH WHEN CODE CHANGES (Maxed)
# ==========================================================
import os
import sys
import time
...
def _watch_for_changes(interval=2):
    """Background thread that watches all project files and triggers reload."""
    mtimes = {}
    while True:
        changed = False
        for root, _, files in os.walk(os.getcwd()):
            for fname in files:
                if not fname.endswith((".py", ".json", ".yaml", ".toml")):
                    continue
        ...
        if changed:
            print("🔄 Code change detected! Auto-refreshing Streamlit app...")
            # Use Streamlit's rerun function (safe reload)
            st.rerun()
        time.sleep(interval)

# Start watcher thread only once
if "auto_reloader" not in st.session_state:
    t = threading.Thread(target=_watch_for_changes, daemon=True)
    t.start()
    st.session_state["auto_reloader"] = True
    print("✅ Auto-reload thread started.")
